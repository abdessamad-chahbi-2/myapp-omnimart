...
<!-- <h1 style="color:red;">THIS IS front.blade.php ✅</h1> -->
...
async function setFingerprintCookie() {
    const existingFingerprint = document.cookie
        .split('; ')
        .find(row => row.startsWith('client_fingerprint='));

    if (!existingFingerprint) {
        const fingerprint = await ThumbmarkJS.getFingerprint();
        document.cookie = `client_fingerprint=${fingerprint}; path=/; max-age=${60*60*24*365}`;
        console.log("Nouvelle empreinte stockée :", fingerprint);
    } else {
    ...
    // Stockage ou mise à jour du cookie IP (1 an)
    document.cookie = `client_public_ip=${publicIp}; path=/; max-age=${60*60*24*365}`;
    console.log("IP publique mise à jour :", publicIp);
}

setFingerprintCookie();

    // Puis continue tes requêtes axios normalement, pas besoin d’intercepteur
    axios.get('/')
        .then(resp => console.log('Test axios:', resp))
